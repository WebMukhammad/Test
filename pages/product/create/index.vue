    ...
    <div class="create__main">
      <div class="create__content">
        <ValidationObserver ref="observerRef" v-slot="{ invalid }">
          <Form width-title="180" @submit="onSubmit">
            ...
            <Notice v-if="isUser && submited">
              <h4 class="h4 mb-7">Товар добавлен в тестовом виде</h4>
              <p class="p mb-10">
                Без активации аккаунта с
                <nuxt-link to="/company" class="link link_blue link_wu">помощью подтверждения данных о компании</nuxt-link>
                товар не попадет в каталог на сайте и будет виден лишь в вашем личном кабинете. После активации аккаунта все товары попадут
                на проверку к модератору и после неё появятся в каталоге.
              </p>
              <h5 class="h5">
                <nuxt-link :to="activeUrl && activeUrl" class="link link_blue link_wu">К списку товаров »</nuxt-link>
              </h5>
            </Notice>
            <div class="create__total">
              <Button v-if="submited" size="49" theme="submited" icon="tick" disabled> Товар добавлен </Button>
              <Button v-else size="49" theme="red" :disabled="loading"> Добавить товар </Button>
      ...
      <div class="create__hint-sticky">
        <Hint title="Используйте хоткеи для продуктивной работы">
          <div class="p">По полям формы можно перемещаться через Tab, выбирать пункты в раскрывающихся списках можно через стрелочки.</div>
          <!-- <nuxt-link to="/" class="link link_wu link_blue p"> Больше информации в разделе «Помощь» </nuxt-link> -->
        </Hint>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async postData() {
      try {
        this.error = false
        const data = await this.$api.product.create(this.field)
        this.submited = true
        ...
        this.activeUrl = `/product?state=checking&id=${data.id}`
        this.isSeller && this.$router.push(this.activeUrl)
      } catch (e) {
        console.log('Ошибка при создании товара', e)
        this.error = e.response.data.errors || 'Ошибка, попробуйте создать товар позже'
      }
    },
    onChange(id, { props }) {
